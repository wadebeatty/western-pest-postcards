/**
 * Green Springs Batch — First 139 leads
 * Builds front+back postcards, uploads to catbox, sends via Lob API
 */
...
// Load first 139 green springs leads
const allLeads = JSON.parse(fs.readFileSync('./green-springs-leads.json', 'utf8'));
const PEOPLE = allLeads.slice(0, 139).map(r => ({
...
console.log(`Loaded ${PEOPLE.length} leads`);
...
function fetchBestSV(a, c, s) {
  ...
  return Promise.all(hs.map(h => new Promise((res) => {
    ...
    const ch = [];
    https.get(u, (r) => { r.on('data', d => ch.push(d)); r.on('end', () => { const b = Buffer.concat(ch); res({ buf: b.length > 12000 ? b : null, size: b.length }); }); }).on('error', () => res({ buf: null, size: 0 }));
...
function upload(f) {
  try {
    const r = execSync(`curl -s -F "reqtype=fileupload" -F "fileToUpload=@${f}" https://catbox.moe/user/api.php`, { timeout: 30000 }).toString().trim();
...
function lobSend(frontUrl, backUrl, p) {
  return new Promise((resolve, reject) => {
    const body = new URLSearchParams({
      ...
      'to[name]': p.name,
      'to[address_line1]': p.addr,
      'to[address_city]': p.city,
      'to[address_state]': p.st,
      'to[address_zip]': p.zip,
      'to[address_country]': 'US',
      'from[name]': 'Western Pest Control',
      'from[address_line1]': '553 W. Calle Del Sol Ste A101',
      'from[address_city]': 'Washington',
      'from[address_state]': 'UT',
      'from[address_zip]': '84780',
...
async function buildOne(p, i) {
  ...
  const ff = path.join(OUT, `${String(i + 1).padStart(3, '0')}-${slug}-front.jpg`);
  const bf = path.join(OUT, `${String(i + 1).padStart(3, '0')}-${slug}-back.jpg`);
  ...
  const tmpF = '/tmp/gs-front-tmp.jpg';
  ...
  const tmpB = '/tmp/gs-back-tmp.jpg';
  ...
  if (!fu || !bu) throw new Error(`Upload failed for ${p.name}`);

  const lobId = await lobSend(fu, bu, p);
  console.log(`✓ ${i + 1}/139 ${p.name} (${td}) — Lob: ${lobId}`);
  return { name: p.name, addr: p.addr, lobId, fu, bu };
}
...
async function run() {
  ...
  for (let i = 0; i < PEOPLE.length; i++) {
    try {
      ...
      console.error(`✗ ${i + 1}/139 ${PEOPLE[i].name} — ${e.message}`);
      failed++;
    }
    await new Promise(x => setTimeout(x, 600));
  }
  fs.writeFileSync('/tmp/green-springs-lob-results.json', JSON.stringify(results, null, 2));
  console.log(`\n✅ DONE — ${results.length} sent, ${failed} failed`);
  console.log(`Results saved to /tmp/green-springs-lob-results.json`);
}
