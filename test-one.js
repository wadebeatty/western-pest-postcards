...
async function buildOne(p, i) {
  const ff = path.join(OUT, `test-front.jpg`);
  const bf = path.join(OUT, `test-back.jpg`);
  ...
  await sharp(tmpF).resize(1100,600,{fit:'fill'}).jpeg({quality:95}).toFile(ff);
  ...
  await sharp(tmpB).resize(1100,600,{fit:'fill'}).jpeg({quality:95}).toFile(bf);

  const fu = upload(ff), bu = upload(bf);
  console.log(`FRONT: ${fu}`);
  console.log(`BACK:  ${bu}`);
}

const jenny = { name: 'JENNY PETERSON', addr: '611 Quail Ridge Dr', city: 'Washington', st: 'UT', zip: '84780' };
buildOne(jenny, 0).catch(console.error);
