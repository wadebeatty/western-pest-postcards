/**
 * Retry Sharon Henrie (index 22) — the one miss from green springs batch
 */
...
const OUT = '/tmp/green-springs-postcards';
fs.mkdirSync(OUT, { recursive: true });

const W = 1100, H = 600;
const PHONE = '(435) 635-1456';
const _d = new Date(); _d.setDate(_d.getDate() + 21);
const DEADLINE = _d.toLocaleDateString('en-US', { month: 'long', day: 'numeric', timeZone: 'America/Denver' }).toUpperCase();

const ALL_REVIEWS = [
  { text: '"Not all heroes wear capes. Some wear Western Pest Control pinstriped shirts."', author: 'Mark Z., St. George' },
...
function wrapLines(text, maxLen) {
  ...
  for (const w of words) { if ((line + ' ' + w).trim().length <= maxLen) { line = (line + ' ' + w).trim(); } else { if (line) lines.push(line); line = w; } }
...
function fetchBestSV(a, c, s) {
  ...
  return Promise.all(hs.map(h => new Promise((res) => {
    ...
    const ch = []; https.get(u, (r) => { r.on('data', d => ch.push(d)); r.on('end', () => { const b = Buffer.concat(ch); res({ buf: b.length > 12000 ? b : null, size: b.length }); }); }).on('error', () => res({ buf: null, size: 0 }));
  }))).then(r => { const v = r.filter(x => x.buf); v.sort((a, b) => b.size - a.size); return v.length ? v[0].buf : null; });
}
...
function uploadWithRetry(f, attempts = 3) {
  for (let i = 0; i < attempts; i++) {
    try {
      const r = execSync(`curl -s -F "reqtype=fileupload" -F "fileToUpload=@${f}" https://catbox.moe/user/api.php`, { timeout: 45000 }).toString().trim();
      if (r.startsWith('https://')) return r;
      console.log(`Upload attempt ${i+1} got non-URL:`, r.substring(0, 100));
    } catch (e) { console.log(`Upload attempt ${i+1} failed:`, e.message); }
    if (i < attempts - 1) { try { execSync('sleep 4'); } catch(e) {} }
  }
  return null;
}

function lobSend(frontUrl, backUrl, p) {
  return new Promise((resolve, reject) => {
    const body = new URLSearchParams({
      'description': `Green Springs — ${p.name}`,
      'to[name]': p.name, 'to[address_line1]': p.addr, 'to[address_city]': p.city,
      'to[address_state]': p.st, 'to[address_zip]': p.zip, 'to[address_country]': 'US',
      'from[name]': 'Western Pest Control', 'from[address_line1]': '553 W. Calle Del Sol Ste A101',
      'from[address_city]': 'Washington', 'from[address_state]': 'UT',
      'from[address_zip]': '84780', 'from[address_country]': 'US',
      'front': frontUrl, 'back': backUrl, 'size': '6x11',
      'mail_type': 'usps_first_class', 'use_type': 'marketing'
    }).toString();
    const auth = Buffer.from(`${LOB_KEY}:`).toString('base64');
    const options = { hostname: 'api.lob.com', path: '/v1/postcards', method: 'POST', headers: { 'Authorization': `Basic ${auth}`, 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(body), 'Lob-Version': '2020-02-11' } };
    const req = https.request(options, (res) => { let data = ''; res.on('data', d => data += d); res.on('end', () => { try { const parsed = JSON.parse(data); if (parsed.id) resolve(parsed.id); else reject(new Error(data)); } catch (e) { reject(new Error(data)); } }); });
    req.on('error', reject); req.write(body); req.end();
  });
}

(async () => {
  const allLeads = JSON.parse(fs.readFileSync('./green-springs-leads.json', 'utf8'));
  const raw = allLeads[22];
  const p = { name: raw.firstName + ' ' + raw.lastName, addr: raw.addr, city: raw.city, st: raw.state || raw.st, zip: raw.zip };
  console.log('Retrying:', p.name, '|', p.addr, p.city, p.st, p.zip);

  const i = 22;
  const ff = path.join(OUT, '023-sharon-henrie-front.jpg');
  const bf = path.join(OUT, '023-sharon-henrie-back.jpg');
  ...
  const en = esc(p.name), al = esc(p.addr + ', ' + p.city + ', UT');
  ...
  await sharp({ create: { width: W, height: H, channels: 3, background: { r: 255, g: 255, b: 255 } } }).composite(bl).jpeg({ quality: 93 }).toFile('/tmp/gs-retry-back-tmp.jpg');
  await sharp('/tmp/gs-retry-back-tmp.jpg').resize(3375, 1875, { fit: 'fill' }).jpeg({ quality: 95 }).toFile(bf);

  console.log('Images built, uploading...');
  const fu = uploadWithRetry(ff);
  const bu = uploadWithRetry(bf);
  if (!fu || !bu) { console.error('Upload failed after 3 attempts'); process.exit(1); }

  console.log('Uploaded, sending to Lob...');
  const lobId = await lobSend(fu, bu, p);
  console.log('✅ 139/139 Sharon Henrie — Lob:', lobId);
